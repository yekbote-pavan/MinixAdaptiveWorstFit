 ...
 */
  ...
  mem = alloc_pages(clicks, memflags); //returns starting mem for the pages to be assigned
  if(mem == NO_MEM) {
...
static phys_bytes alloc_pages(int pages, int memflags)
{
	...
	if(memflags & PAF_LOWER16MB)
		maxpage = boundary16 - 1; // starting address of the last block in memory
	else if(memflags & PAF_LOWER1MB)
		maxpage = boundary1 - 1;	// starting address of the last block in memory, 1MB frames?
	else {
		/* no position restrictions: check page cache */
		// only if requested pages == 1
		// will have to change for worst fit
		if(pages == 1) {
			while(free_page_cache_size > 0) {	// free pages in memory > 0
				i = free_page_cache[free_page_cache_size-1];
				if(page_isfree(i)) {		// found a free page
	...
	//if(lastscan < maxpage && lastscan >= 0)  // lastscan is static, marks start of the last block we looked for, in the prev call
	//	startscan = lastscan;
//	else	startscan = maxpage; // start scanning from last page in memory
	
	// modify here
	// add a loop, always scan from max to 0, 
	// maintain a max_mem and a max run_length
	// use the max ones when we finish a full scan of memory
//	if(mem == NO_MEM)
...
//	lastscan = mem;
//************** STart ***************
	int largest_block = pages;
	phys_bytes_max_mem = NO_MEM;
	int max_runlength = -1;
	int count = MAXMEM32;
	int large_fail;
	if(mem === NO_MEM)
	{
		while(count-- > 0 && largest_block < maxpage)
		{
			startscan = maxpage;
			max_mem = NO_MEM;
			
			mem = findbit(0,startscan,largest_block,memflags,&run_length);
			if( mem != NO_MEM && mem != 4294967294 && run_length > max_runlength)
			{
				max_runlength = run_length;
				max_mem = mem;
			}
			else
			{
				large_fail = largest_block;
				while(max_runlength < large_fail)
				{
					if( mem != 4294967294)
					{
						mem = findbit(0, startscan, max_runlength++,memflags,&run_length);
					}
					else
					{
						break;
					}
					if( mem != NO_MEM && mem != 4294967294)
					{
						max_mem = mem;
						max_runlength = run_length;
					}
	...
	for(i = mem; i < mem + pages; i++) {
		// mark these pages as  no longer free
		UNSET_BIT(free_pages_bitmap, i);
	}

	if(memflags & PAF_CLEAR) {
		int s;
		// copies data into these pages
