 ...
 */
  ...
  int q;				/* iterate over queues */
	int i =0; // iterate over user processes
	int min_frame = INT_MAX;
	struct time_t min_time = INT_MAX;
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	...
	if(q == USER_Q)	
	{
		...
		int min_last_ran = INT_MAX;
		while(temp != NULL)
		{
			if(temp->last_ran < min_last_ran)
			{
				rp = temp;
				min_time = temp->time_ran;
				min_last_ran = temp->last_ran;
			}
			if( temp -> last_ran == min_last_ran && temp -> time_ran < min_time)
			{
				rp = temp;
				min_time = temp -> time_ran;
			}
