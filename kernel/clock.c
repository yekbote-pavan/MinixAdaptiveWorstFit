...
#include "clock.h"
#define FRAME_TICKS 300 // assuming 60 ticks -> 1s, 5 sec frames
#ifdef USE_WATCHDOG
#include "watchdog.h"
#endif
...
int timer_int_handler(void)
{
	...
	if(realtime%FRAME_TICKS == 0)
	{
		cur_frame_start = realtime; // start of current frame
	}
	struct proc * p, * billp;
	...
	if(p->which_frame != cur_frame_start)
	{
			// first time in this 5 second frame
			p->which_frame = cur_frame_start;
			p->time_consumed_in_frame = 0;
	}

	p->p_user_time++;
	//increment time_consumed_in_frame
	p->time_consumed_in_frame++;	
